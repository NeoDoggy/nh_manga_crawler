...
mkdir ./$booknum
wget -O ./$booknum/temp.html https://nhentai.net/g/$booknum/
imgname=$(grep -oP 'galleries\/\K\w+' ./$booknum/temp.html | head -1)
i=$first
...
do
    ...
    echo "======== Now page:${i} ========"
    echo ""
    wget -O ./$booknum/$i.png https://i.nhentai.net/galleries/$imgname/$i.png
    expng=$(test -s $booknum/$i.png && echo "exist" || echo "Not exist" )
    if [ "$expng" == "Not exist" ];then
        echo "======== At page ${i} found no png , changing to jpg ========"
        echo ""
        wget -O ./$booknum/$i.png https://i.nhentai.net/galleries/$imgname/$i.jpg
        exjpg=$(test -s $booknum/$i.png && echo "exist" || echo "Not exist" )
        i=$(($i+1))
    else
